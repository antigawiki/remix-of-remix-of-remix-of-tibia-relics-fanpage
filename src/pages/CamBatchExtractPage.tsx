  ...
  const processAll = useCallback(async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const totalBatches = Math.ceil(tileCount / TILE_RPC_BATCH) + Math.ceil(spawnCount / SPAWN_RPC_BATCH);
        let batchesDone = 0;

        // Upload tiles in parallel batches
        const tileBatches: Array<{ start: number; batch: any[] }> = [];
        for (let j = 0; j < tileCount; j += TILE_RPC_BATCH) {
          const batch = tiles.slice(j, j + TILE_RPC_BATCH).map(([key, items]) => {
            const [x, y, z] = key.split(',').map(Number);
            return { x, y, z, items };
          });
          tileBatches.push({ start: j, batch });
        }

        for (let j = 0; j < tileBatches.length; j += PARALLEL_UPLOADS) {
          if (abortRef.current) break;
          const chunk = tileBatches.slice(j, j + PARALLEL_UPLOADS);

          const results = await Promise.allSettled(
            chunk.map(async ({ start, batch }) => {
              for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
                const { data, error } = await supabase.rpc('merge_cam_tiles_batch' as any, { tiles: batch });
                if (!error) return (data as number) || batch.length;
                console.error(`[BatchExtract] Tile batch ${start} attempt ${attempt + 1}:`, error.message);
          ...
          for (const r of results) {
            if (r.status === 'fulfilled') {
              ...
              tileFailed += chunk[0]?.batch.length || 0;
            }
            batchesDone++;
          }

          setFiles(prev => prev.map((f, idx) =>
            idx === i ? { ...f, progress: Math.round((batchesDone / totalBatches) * 100) } : f
          ));
        }

        // Upload spawns via batch RPC with retries
        for (let j = 0; j < spawnCount; j += SPAWN_RPC_BATCH) {
          ...
          for (let attempt = 0; attempt < MAX_RETRIES && !success; attempt++) {
            ...
            if (error) {
              console.error(`[BatchExtract] Spawn batch ${j} attempt ${attempt + 1}:`, error.message);
              if (attempt < MAX_RETRIES - 1) await new Promise(r => setTimeout(r, 1000 * (attempt + 1)));
            } else {
              success = true;
            }
        ...
        console.log(`[BatchExtract] ${files[i].file.name}: ${tileSaved} tiles salvos, ${tileFailed} falharam, ${spawnCount} spawns`);
        if (tileFailed > 0) {
          toast.warning(`${files[i].file.name}: ${tileFailed} tiles falharam no upload`);
        }
